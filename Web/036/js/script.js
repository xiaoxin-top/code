...
async function Draw() {
  ...
  if (!t) {
    ...
    lineFaceI = (X1, Y1, Z1, X2, Y2, Z2, facet, autoFlipNormals = false, showNormals = false) => {
      ...
      let p2_ = -(Math.acos((Y2_ - Y1_) / (Math.hypot(X2_ - X1_, Y2_ - Y1_, Z2_ - Z1_) + 0.001)) + Math.PI / 2);
      let isc = false,
    ...
    Cube = (size) => {
      for (CB = [], j = 6; j--; CB = [...CB, b]) for (b = [], i = 4; i--; ) b = [...b, [(a = [S((p = ((Math.PI * 2) / 4) * i + Math.PI / 4)), C(p), 2 ** 0.5 / 2])[j % 3] * (l = j < 3 ? size / 1.5 : -size / 1.5), a[(j + 1) % 3] * l, a[(j + 2) % 3] * l]];
      return CB;
    };
      ...
      if (sphereize) {
        ...
        shape = shape.map((v) => {
          v = v.map((q) => {
            ...
            Y /= d;
            Z /= d;
            X *= size * 0.75 * ip1 + d * ip2;
            Y *= size * 0.75 * ip1 + d * ip2;
            Z *= size * 0.75 * ip1 + d * ip2;
            return [X, Y, Z];
          });
          return v;
        });
    ...
    LsystemRecurse = (size, splits, p1, p2, stem, theta, LsystemReduction, twistFactor) => {
      ...
      let p2a = -Math.acos((Y2 - Y1) / (Math.hypot(X2 - X1, Y2 - Y1, Z2 - Z1) + 0.0001)) + Math.PI;
        ...
        for (j = rw; j--; )
          for (let i = cl; i--; ) {
            if (parts > 1) {
              ...
              p2a = Math.PI / 2 + Math.acos((ay2 - ay1) / (Math.hypot(ax2 - ax1, ay2 - ay1, az2 - az1) + 0.001));
              ...
              p2b = Math.PI / 2 + Math.acos((ay2b - ay1b) / (Math.hypot(ax2b - ax1b, ay2b - ay1b, az2b - az1b) + 0.001));
            }
            a = [];
    ...
    obj = subDividedTetrahedron(5, 4, 4); //await loadOBJ('https://srmcgann.github.io/objs/brain.obj', 3, 0,5,1, 0,0,Math.PI)
    ...
    spawnSplosion = () => {
      ...
      Y = (Rn() - 0.5) * G;
      Z = (Rn() - 0.5) * G;
      for (let m = 50; m--; ) spawnSparks(X, Y, Z, 3);
      for (m = 100; m--; ) {
        p1 = Rn() * Math.PI * 2;
        p2 = Rn() < 0.5 ? Math.PI - (Rn() ** 0.5 * Math.PI) / 2 : (Rn() ** 0.5 * Math.PI) / 2;
        vx = S(p1) * S(p2) * iSplodev;
        vy = C(p2) * iSplodev;
        vz = C(p1) * S(p2) * iSplodev;
        splosions = [...splosions, [X + vx * 2, Y + vy * 2, Z + vz * 2, vx, vy, vz, 1, []]];
      }
    };

    sparks = [];
    iSparksv = 0.05;
    spawnSparks = (X, Y, Z, ov = 1) => {
      for (m = 5; m--; ) {
        p1 = Rn() * Math.PI * 2;
        p2 = Rn() < 0.5 ? Math.PI - (Rn() ** 0.5 * Math.PI) / 2 : (Rn() ** 0.5 * Math.PI) / 2;
        vx = S(p1) * S(p2) * iSparksv * ov;
        vy = C(p2) * iSparksv * ov;
        vz = C(p1) * S(p2) * iSparksv * ov;

        sparks = [...sparks, [X + vx / 2, Y + vy / 2, Z + vz / 2, vx, vy, vz, 1]];
      }
    };
  }

  (oX = 0), (oY = 0), (oZ = Math.min(24, Math.max(8, (0.3 + S(t)) * 100)));
  (Rl = 0), (Pt = 0.2 - S(t / 3)), (Yw = Math.PI / 2 - C(t / 4) * 4);
  ...
  ST.map((v) => {
    ...
    R(Rl, Pt, Yw, 1);
    if (Z > 0) {
      if ((x.globalAlpha = Math.min(1, (Z / 1e4) ** 2)) > 0.1) {
  ...
  obj.map((v) => {
    ...
    stroke("#f001", "#ff000006", 3, true);
  });

  if (!(((t * 60) | 0) % splodeFreq)) spawnSplosion();

  splosions = splosions.filter((v) => v[6] > 0);
  rv = 0.425;
  anticipation = 1.33;
  variation = 0;
  splosions.map((v) => {
    ...
    ty1 = v[1] += v[4];
    tz1 = v[2] += v[5];
    tx2 = tx1 + v[3] * anticipation;
    ty2 = ty1 + v[4] * anticipation;
    tz2 = tz1 + v[5] * anticipation;
    obj.map((q) => {
      ax = ay = az = 0;
      q.map((n) => {
        ax += n[0];
      ...
      ay /= q.length;
      az /= q.length;
      d = Math.hypot((a = ax - (tx1 + tx2) / 2), (b = ay - (ty1 + ty2) / 2), (e = az - (tz1 + tz2) / 2));
      if (d < 1.5) {
        if ((l = lineFaceI(tx1, ty1, tz1, tx2, ty2, tz2, q, true, false))) {
          vx = l[1][0];
          vy = l[1][1];
          vz = l[1][2];
          ...
          vy /= d;
          vz /= d;
          v[3] += vx * rv + (Rn() - 0.5) * variation;
          v[4] += vy * rv + (Rn() - 0.5) * variation;
          v[5] += vz * rv + (Rn() - 0.5) * variation;
          //v[6] = 0
          spawnSparks(...l[0]);
        }
      }
    ...
    v[5] *= iSplodev;
    v[7] = [...v[7], [tx1, ty1, tz1]];
    v[7] = v[7].filter((q, j) => v[7].length < 20 || (v[7].length >= 20 && j));
    v[7].map((q, j) => {
      if (j) {
        x.beginPath();
        l = j;
        ...
        R(Rl, Pt, Yw, 1);
        if (Z > 0) x.lineTo(...Q());
        stroke(`hsla(${t * 600 - (360 / v[7].length) * j},99%,${50 + (50 / v[7].length) * j}%,.75)`, "", (j / 1.5) * v[6], false);
      }
    });
    v[6] -= 0.01;
  });

  sparks = sparks.filter((v) => v[6] > 0.1);
